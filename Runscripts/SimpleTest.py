...
def main():
    ...
    # a = np.array([[1,0,1], [0,1,2], [1,2,0]])
    # a = cf.getHessenBergForm(a)
    # print(a)
    A2 = cf.getHessenBergForm(A)
    # Get the first decomposition of a into QR
    # Q, R = cf.getQR(A)
    eigenVals, eigenVecs = cf.getQREigens(A, cntMax=1e5)
    eValsError, eVecsError = cf.calculateEigenError(eigenVals, eigenVecs, A)
    print("Errors", eValsError, eValsError)
    c = input()
    eVal, eigenVec = np.linalg.eig(A)
    # eVal2, eigenVec2 = np.linalg.eig(A2)
    print("This")
    print("QR method")
    print(eigenVals, eigenVecs)
    print("should match")
